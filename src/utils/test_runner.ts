...
import { ZERO_ADDRESS_STRING, hexStrToBuf32, makeFakeTransaction, toHexString } from "./misc";
...
export class VMTestRunner {
    ...
    async harveyStepToTransaction(step: BaseTestStep): Promise<TypedTransaction> {
        ...
        if (senderAccount === undefined) {
            throw new Error(`Unable to get account for address ${senderAddress.toString()}`);
        }
    ...
    private async _runTxInt(tx: TypedTransaction, block: Block): Promise<RunTxResult> {
        const txHash = toHexString(tx.hash());

        this._txs.push(tx);

        this._stateRootBeforeTx.set(txHash, this.vm.stateManager.shallowCopy(false));
