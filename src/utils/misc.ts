...
export function hexStrToBuf32(v: UnprefixedHexString): Uint8Array {
    return padStart(hexToBytes(v), 32);
}
...
export function ppEvmStack(stack: Stack): string {
    return stack.map(bytesToHex).join("\n");
}
 ...
 */
export function bigEndianBytesToBigint(buf: Uint8Array): bigint {
    let res = BigInt(0);
 ...
 */
export function bigEndianBytesToNumber(buf: Uint8Array): number {
    const bigintRes = bigEndianBytesToBigint(buf);
    assert(
